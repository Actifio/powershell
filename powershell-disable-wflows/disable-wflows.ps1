...
[string]$workflowname)
{
    ...
    if ($wflowstat -eq $false) {
        write-host "Disabling workflow id ( $workflow_id ) for $workflowname"
        write-host "udstask chworkflow -disable true $workflow_id"  
        udstask chworkflow -disable true $workflow_id | out-null
    } else {
        write-host "Workflow id ( $workflow_id ) for $workflowname is already disabled!!"    
    }
}

foreach ($wflowname in $wflowlist.split(",")) {
  Disable-WorkflowID $wflowname
}
