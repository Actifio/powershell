...
function select-target {
    ...
    foreach ($a in ($deduppools | sort usedpct).appliance)
    {
        ...
        # Check dedup pool is below warning threshold
        if (($pools.$a | where pooltype -eq "dedup").warnstate -eq "true")
        {
            log-output -logfile $logfile -debugonly -message ("Appliance $a dedup pool is over warning threshold and therefore it cannot be the target.")
            continue
        }

        # Check DR appliance dedup pool is below warning threshold
        if (($pools.(($appliances | where appliance -eq $a).drappliance) | where pooltype -eq "dedup").warnstate -eq "true")
        {
            log-output -logfile $logfile -debugonly -message ("DR Appliance " + ($appliances | where appliance -eq $a).drappliance + " dedup pool is over warning threshold and therefore appliance $a cannot be the target.")
            continue
        }
...
foreach ($a in $appliances)
{
    ...
    $localpools=@()
    foreach ($t in ($temppools | where pooltype -notlike "*vault"))
    {
        ...
        if ( $pooldetail.pooltype -eq "dedup")
        {
            $dpool = New-Object System.Object
            $dpool | Add-Member -type NoteProperty -name appliance -value $a.appliance
            $dpool | Add-Member -type NoteProperty -name usedpct -value $pool.usedpct
            $deduppools += $dpool
        }
    }
    ...
    #######################################################
    # Logout of this appliance and in to the DR appliance #
    #######################################################

    log-output -logfile $logfile -debugonly -message ("Logging out of appliance " + $a.appliance)
    Disconnect-Act | Out-Null

    log-output -logfile $logfile -debugonly -message ("Logging in to appliance " + $a.drappliance)
    if (act-login -appliance $a.drappliance -user $a.druser -pwfile $a.drpwfile)
    {
        log-output -logfile $logfile -message ("WARNING: Login to " + $a.drappliance + " failed")
        $ineligibleappliances.($a.appliance)="true"
        break
    }

    #################################
    # Get DR system dedup pool info #
    #################################

    log-output -logfile $logfile -debugonly -message ("Collecting dedup pool information from DR appliance " + $a.drappliance)
    $temppools = udsinfo lsdiskpool

    $localpools=@()
    foreach ($t in ($temppools | where pooltype -eq "dedup"))
    {
        $pooldetail = udsinfo lsdiskpool $t.name
        $pool = New-Object System.Object
        $pool | Add-Member -type NoteProperty -name name -value $pooldetail.name
    ...
    $pools.($a.drappliance) = $localpools
...
{
    # Select a taget appliance based on dedup space, but accounting for other limits in snap pool
